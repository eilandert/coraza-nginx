    ...
    server {
        ...
        location /nobuffering_detectiononly {
            coraza_rules '
                ...
                SecRequestBodyAccess On
                SecAction "id:1,phase:1,pass,nolog,ctl:requestBodyProcessor=URLENCODED"
                SecRule REQUEST_BODY "@rx BAD BODY" "id:12,phase:2,deny,log,status:403"
            ';
            proxy_request_buffering off;
            proxy_http_version 1.1;
            proxy_pass http://127.0.0.1:%%PORT_8081%%;
...
like(http_req_body('POST', '/nobuffering', 'GOOD BODY' x 40),
...
like(http_req_body('POST', '/nobuffering', ('GOOD BODY' x 40) . 'BAD BODY'),
	qr/^HTTP.*403/,
...
like(http_req_body('POST', '/nobuffering_detectiononly', ('GOOD BODY' x 40) . 'BAD BODY'),
	qr/TEST-OK-IF-YOU-SEE-THIS/,
