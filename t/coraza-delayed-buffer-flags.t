    ...
    server {
        ...
        location /large {
            ...
            coraza_rules '
                ...
                SecResponseBodyAccess On
                SecResponseBodyMimeType text/plain
                SecRule RESPONSE_BODY "@rx NEVER MATCHES THIS" "id:172,phase:4,deny,log,status:403"
            ';
        }
...
# the body is read in small, paced chunks so the response outruns nginx's
# proxy buffers and forces the upstream side to spill to a temp file.
sub read_full_response {
    ...
    eval {
        ...
        if ($opt{slow}) {
            ...
            # reader downstream: nginx fills proxy_buffers and then has to
            # write the remainder to a temp file.
...
sub http_daemon {
    my ($port, $recycled_body, $large_body) = @_;
    ...
    while (my $client = $server->accept()) {
        ...
        my $body = ($uri =~ m{^/large}) ? $large_body : $recycled_body;
        my $type = 'text/plain';
        ...
        my $chunk_size = ($uri =~ m{^/large}) ? 65536 : 256;
