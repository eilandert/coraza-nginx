...
like($pre_access,
	qr/while \(offset < body_size\).*?coraza_append_request_body.*?ret\s*=\s*ngx_http_coraza_process_intervention\(ctx,\s*r,\s*0,\s*1\);.*?if \(ret < 0\) \{.*?goto done;.*?if \(ret > 0\) \{.*?goto done;/s,
